class CmRiskType < ActiveRecord::Base
  belongs_to :project
  ...
  private
  def check_integrity
    if CmRisk.find(:first, :conditions => ["type_id=?", self.id])
      raise "Can't delete type because there are Risks using it!!"
    end
  end
  ...
  def just_one_default
    if(self.is_default)
      if new_record?
        @riskType=CmRiskType.find(:first,
      ...
      errors.add(:is_default, "already assigned to " + @riskType.name) unless (@riskType.nil?)
    end
  end
end